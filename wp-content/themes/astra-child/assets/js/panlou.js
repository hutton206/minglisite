...
function calcBazi(y, mo, d, tst_h) {
  ...
  // 夜子時（23:00～24:00）：時干用隔天日干推算，日柱仍顯示當天
  let hourDayGanIdx = day.ganIdx;
  if (tst_h >= 23) {
    const tomorrow = new Date(Date.UTC(y, mo - 1, d + 1));
    const nextDay = calcDayGanzhi(tomorrow.getUTCFullYear(), tomorrow.getUTCMonth() + 1, tomorrow.getUTCDate());
    hourDayGanIdx = nextDay.ganIdx;
  }
  const hour = calcHourGanzhi(tst_h, hourDayGanIdx);
